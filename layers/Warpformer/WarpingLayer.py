...
class Almtx(nn.Module):
    ...
    def cal_new_bound(self, Rl, Rr, gamma):
        ...
        arange_l = torch.linspace(0, 1, S + 1, device=gamma.device)[:-1]
        new_Rl = torch.min(vl, arange_l).unsqueeze(-1).expand(B, S, L)
        ...
        arange_r = torch.linspace(0, 1, S + 1, device=gamma.device)[1:]
        tmp_Rr = torch.max(vr, arange_r).unsqueeze(-1).expand(B, S, L)
        new_Rr = tmp_Rr.clone()
    ...
    def forward(self, input_seq, mask):
        ...
        boundaries = torch.linspace(0, 1, self.S + 1, device=gamma.device)
        Rl_base = boundaries[:-1]
        Rl = Rl_base.unsqueeze(0).unsqueeze(-1).expand(1, self.S, L)
        Rr_base = boundaries[1:]
        Rr = Rr_base.unsqueeze(0).unsqueeze(-1).expand(1, self.S, L)
