...
#     c =  global_out / (global_out_sum + 1e-9)
#     return c
def softmax(src: Tensor, index, num_nodes: int):
    # num_nodes 已经是整数了，maybe_num_nodes 主要是为了安全和推断
    N = maybe_num_nodes(index, num_nodes=num_nodes)

    if src.numel() == 0:
        return src.new_zeros((N, *src.shape[1:]))

    src_max = scatter(src, index, dim=0, dim_size=N, reduce='max')
    src_max_expanded = src_max[index]

    out = src - src_max_expanded
    out = out.exp()

    out_sum = scatter(out, index, dim=0, dim_size=N, reduce='sum')
    out_sum_expanded = out_sum[index]

    return out / (out_sum_expanded + 1e-9)
...
class Intra_Inter_Patch_Graph_Layer(MessagePassing):
    ...
    def message(self, x_j, x_i, edge_index_i, edges_temporal, edge_same_time_diff_var, edge_diff_time_same_var, edge_diff_time_diff_var, n_layer, size_i):
        ...
        for i in range(self.n_heads):
            ...
            attention_norm = softmax(attention, edge_index_i, size_i)
