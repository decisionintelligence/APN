...
from utils.tools import EarlyStopping, test_params_and_flops, test_peak_gpu_memory, test_training_step_time, test_inference_step_time
...
class Exp_Main(Exp_Basic):
    ...
    def train(self) -> None:
        ...
        base_path = Path(self.configs.checkpoints)
        # 如果指定了消融实验名称，则添加该目录作为一级目录
        if self.configs.ablation_name:
            base_path = base_path / self.configs.ablation_name
        path = base_path / self.configs.dataset_name / self.configs.model_name / self.configs.model_id / f"{self.configs.seq_len}_{self.configs.pred_len}" / self.configs.subfolder_train / f"iter{self.configs.itr_i}"
    ...
    def test(self) -> None:
        ...
        task_key = task_key_mapping.get(self.configs.task_name, self.configs.task_name)

        if self.configs.test_flop:
            self.configs.batch_size = 1
            logger.info("Batch size overwritten to 1 for FLOPs test.")
            model = self._build_model()

            # --- 核心修改：为所有必需的输入定义形状并创建虚拟张量 ---
            bs = self.configs.batch_size
            seq_len = self.configs.seq_len
            pred_len = self.configs.pred_len
            enc_in = self.configs.enc_in
            c_out = self.configs.c_out

            if self.configs.features in ['M', 'MS']:
                n_time_features = 4
            elif self.configs.features == 'S':
                n_time_features = 1
            else:
                n_time_features = 0
                logger.warning("Could not determine number of time features for FLOPs test. Assuming 0.")

            # 创建一个包含所有必需输入的字典。
            # 字典的顺序很重要，第一个('x')将被ptflops用作改变形状的主输入。
            # 其他的将作为固定的关键字参数传递。
            dummy_inputs = {
                'x': torch.randn(bs, seq_len, enc_in, device=self.device),
                'x_mark': torch.randn(bs, seq_len, n_time_features, device=self.device),
                'x_mask': torch.ones(bs, seq_len, enc_in, device=self.device),

                # 为解码器（Decoder）提供必需的虚拟输入
                'y_mark': torch.randn(bs, pred_len, n_time_features, device=self.device),
                # 即使模型内部可能不直接使用y，它也常常是forward函数的必需参数，用作占位符
                'y': torch.zeros(bs, pred_len, c_out, device=self.device),
                'y_mask': torch.ones(bs, pred_len, c_out, device=self.device)
            }

            test_params_and_flops(
                model=model,
                dummy_inputs=dummy_inputs,  # 传入包含所有虚拟输入的字典
                model_id=self.configs.model_id,
                task_key=task_key
            )
            exit(0)
        ...
        if self.configs.test_train_time:
            # 使用配置中指定的 batch_size
            logger.info(f"Using batch_size={self.configs.batch_size} for training time test.")
            train_data, train_loader = self._get_data(flag='train')
            model = self._build_model()
            criterion = self._select_criterion()
            optimizer = self._select_optimizer(model)
            test_training_step_time(
                model=model,
        ...
        if self.configs.test_inference_time:
            # 使用配置中指定的 batch_size
            logger.info(f"Using batch_size={self.configs.batch_size} for inference time test.")
            test_data, test_loader = self._get_data(flag='test')  # 使用测试集进行推理测试
            model = self._build_model()
            test_inference_step_time(
                model=model,
        ...
        if self.configs.test_gpu_memory:
            logger.info(f"Using batch_size={self.configs.batch_size} for GPU memory test.")
            train_data, train_loader = self._get_data(flag='train')
            batch = next(iter(train_loader))
            batch = {k: v.to(f"cuda:{self.configs.gpu_id}") for k, v in batch.items()}

            model = self._build_model()
            criterion = self._select_criterion()  # <--- 核心修改：创建正确的criterion

            test_peak_gpu_memory(
                model=model,
                batch=batch,
                criterion=criterion,  # <--- 核心修改：传入criterion
                model_id=self.configs.model_id,
                gpu_id=self.configs.gpu_id
            )
            exit(0)
        ...
        if self.configs.checkpoints_test is None:
            ...
            if self.configs.ablation_name:
                base_path = base_path / self.configs.ablation_name
            checkpoint_location = base_path / self.configs.dataset_name / self.configs.model_name / self.configs.model_id / f"{self.configs.seq_len}_{self.configs.pred_len}"
